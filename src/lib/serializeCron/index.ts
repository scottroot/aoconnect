import type { Tag, AnyTag } from "../../types";


export interface SerializeCron {
  interval: string | any;
  tags?: AnyTag[];
}
/**
 ...
 */
export function serializeCron(cron: SerializeCron): Tag[] {
  function parseInterval(interval: any): string {
  ...
  function parseTags(tags: AnyTag[] = []): Tag[] {
    return tags.map((tag) => {
